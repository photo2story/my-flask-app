...
$(function () {
    ...
    $(".clickable-image").on("click", function(){
        ...
        modalImg.src = this.src; // 클릭한 이미지의 src 가져오기
        captionText.innerHTML = this.alt; // 클릭한 이미지의 alt 텍스트 가져오기
    });
...
$(window).on('resize', function() {
    if ($(window).width() < 1200) {
        ...
        $('.scrollable-section').css('width', '100%');
        $('.right-panel').css('flex-direction', 'column');
    } else {
        $('.content-wrapper').css('flex-direction', 'row');
        $('.scrollable-section').css('width', '40%');
        $('.right-panel').css('flex-direction', 'row');
    }
}).trigger('resize');
